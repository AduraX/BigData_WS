...
ARCH_DIR=/vagrant/Archive # /vagrant/Archive $PROJ_DIR/Archive
DATA_DIR=/var/bdData
...
#[** Creating cluster on Vagrant and Checking if the Cluster is ready
sTime1=$(date +%s)
IP_BASE=$(CreateHost $NumMasters $NumSlaves) # $lbAZ $NumMasters $NumSlaves $isBastion $clusterType
#echo "\n\n===== IP_BASE: $IP_BASE  ==========\n\n"
...
if [ $isBastion == "Y" ]; then
  ...
  lbInventory="$lbInventory bastion$lbAZ ansible_ssh_host=$Host_Dns ansible_port=$Host_Port ansible_user=$AnsUSER  ansible_private_key_file=~/.vagrant.d/insecure_private_key\n"
fi
...
for ((i=1; i<=NumMasters; i++)); do
  ...
  lbInventory="$lbInventory master$Psn$lbAZ ansible_ssh_host=$Host_Dns ansible_port=$Host_Port ansible_user=$AnsUSER  ansible_private_key_file=~/.vagrant.d/insecure_private_key\n"
...
for ((i=1; i<=NumSlaves; i++)); do
  ...
  lbInventory="$lbInventory slave$Psn$lbAZ ansible_ssh_host=$Host_Dns ansible_port=$Host_Port ansible_user=$AnsUSER  ansible_private_key_file=~/.vagrant.d/insecure_private_key\n"

  if [ $i -le 3 ]; then lbInvenZK="$lbInvenZK slave$Psn$lbAZ ansible_ssh_host=$Host_Dns ansible_port=$Host_Port ansible_user=$AnsUSER ansible_private_key_file=~/.vagrant.d/insecure_private_key\n"; fi
