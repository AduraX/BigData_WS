...
# export ANSIBLE_HOST_KEY_CHECKING=False && ansible-playbook -i AnsibleInventory k8s_bdPlat.yml
# ssh LogHost
...
platType=${1:-VAG} # platType Options: [VAG | AWS | AZURE | BARE]
...
read doDelete
  if [ $doDelete = "y" ] || [ $doDelete = "Y" ]; then
    ...
    if [ $platType = "VAG" ]; then
      vagNodes=$(echo $(cat Platforms/$platType/VagNodes))
      cd Platforms/$platType
      vagrant destroy -f $vagNodes # vagrant destroy -f 8fb73a6 132914f 9117e74  7a3a018
      cd -
      rm Platforms/$platType/Vagrantfile Platforms/$platType/VagNodes
    elif [ $platType = "AWS" ]; then
      aws cloudformation delete-stack --stack-name $StackName #To delete the stack
      rm Platforms/$platType/template.yaml
    else
      echo delete Azure $StackName
    fi
    rm connect.sh AnsibleInventory Platforms/$platType/StackNameFile
  else
    $ECHO "\n**** No worries! Exiting...\n"
    cd .. && exit
  fi
